 ...
 * Task_NewGameBirchSpeech_GirlOrBoy
 ...
 *  - Otherwise, return to Task_NewGameBirchSpeech_GirlOrBoy.
 *
...
static void Task_NewGameBirchSpeech_GirlOrBoy(u8);
...
static void Task_NewGameBirchSpeechSub_InitPokeBall(u8 taskId)
{
    ...
    CreatePokeballSpriteToReleaseMon(spriteId, gSprites[spriteId].oam.paletteNum, 112, 58, 0, 0, 32, PALETTES_BG, SPECIES_RALTSD);
...
static void Task_NewGameBirchSpeech_WaitForPlayerFadeIn(u8 taskId)
{
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        gTasks[taskId].func = Task_NewGameBirchSpeech_GirlOrBoy;
    }
}

static void Task_NewGameBirchSpeech_GirlOrBoy(u8 taskId)
{
    NewGameBirchSpeech_ClearWindow(0);
    StringExpandPlaceholders(gStringVar4, gText_Birch_GirlOrBoy);
    AddTextPrinterForMessage(TRUE);
...
static u8 NewGameBirchSpeech_CreateRaltsDSprite(u8 x, u8 y)
{
    return CreateMonPicSprite_Affine(SPECIES_RALTSD, SHINY_ODDS, 0, MON_PIC_AFFINE_FRONT, x, y, 14, TAG_NONE);
}
...
static void AddBirchSpeechObjects(u8 taskId)
{
    ...
    lotadSpriteId = NewGameBirchSpeech_CreateRaltsDSprite(100, 0x4B);
