...
static const u16 sHoennToNationalOrder[HOENN_DEX_COUNT - 1] =
{
    ...
    HOENN_TO_NATIONAL(RALTSD),
    HOENN_TO_NATIONAL(KIRLIAD),
    HOENN_TO_NATIONAL(GARDEVOIRD),
    ...
    HOENN_TO_NATIONAL(PLUDENTEE),
    HOENN_TO_NATIONAL(MINUN),
    HOENN_TO_NATIONAL(MIDENTEE),
...
#include "data/pokemon/level_up_learnsets/dream_emerald.h"
...
bool8 PokemonUseItemEffects(struct Pokemon *mon, u16 item, u8 partyIndex, u8 moveIndex, bool8 usedByAI)
{
    ...
    for (i = 0; i < ITEM_EFFECT_ARG_START; i++)
    {
        ...
        case 3:
            ...
            {
                u16 param = ItemId_GetHoldEffectParam(item);
                dataUnsigned = 0;
    ...
    case EVO_MODE_BATTLE_ONLY:
        ...
        for (i = 0; evolutions[i].method != EVOLUTIONS_END; i++)
        {
            ...
            case EVO_LEVEL_FEMALE_LOW_HP:
                if(evolutions[i].param <= level && GetMonGender(mon) == MON_FEMALE) {
                    u32 currHP = GetMonData(mon, MON_DATA_HP, 0);
                    u32 halfMaxHP = GetMonData(mon, MON_DATA_MAX_HP, 0) / 2;
