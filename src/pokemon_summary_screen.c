...
static const u8 sMoveTypeToOamPaletteNum[NUMBER_OF_MON_TYPES + CONTEST_CATEGORIES_COUNT + SOUND_TYPE_VARIATIONS] =
{
    ...
    [TYPE_SOUND] =        PAL_TYPE_SOUND    , // NOTE: Using a vram location that I really dont know if it is unused. hope it's ok
...
static void ExitMovePositionSwitchMode(u8 taskId, bool8 swapMoves)
{
    ...
    u8 firstMoveIndex = sMonSummaryScreen->firstMoveIndex;
    u8 secondMoveIndex = sMonSummaryScreen->secondMoveIndex;
    ...
    if (swapMoves == TRUE)
    {
        ...
        SwapMovesTypeSprites(firstMoveIndex, secondMoveIndex);
...
// We want to always be able to replace moves
...
static void PrintMovePowerAndAccuracy(u16 moveIndex)
{
    ...
    if (moveIndex != 0)
    {
        ...
        {
            ConvertIntToDecimalStringN(gStringVar1, (10 * monFriendship / 20), STR_CONV_MODE_RIGHT_ALIGN, 3);
            text = gStringVar1;
        }
        else if (moveIndex == MOVE_FRUSTRATION)
        {
            ConvertIntToDecimalStringN(gStringVar1, (10 * (MAX_FRIENDSHIP - monFriendship) / 20), STR_CONV_MODE_RIGHT_ALIGN, 3);
            text = gStringVar1;
...
static void SetMonTypeIcons(void)
{
    ...
    SetSpriteInvisibility(SPRITE_ARR_ID_TYPE + 1, TRUE); //This ensures that at least one is always invisible if unused
    ...
    else
    {
        ...
        if (gBaseStats[summary->species].type1 != gBaseStats[summary->species].type2)
        {
            ...
            //SetSpriteInvisibility(id_type + 1, FALSE); //idk why this was there when SetTypeSpritePosAndPal already makes it visible
        }
    }
}
...
static void BufferIvOrEvStats(){
    ...
    switch (sMonSummaryScreen->ivOrEvMode)
    {
    case 0: // iv mode
